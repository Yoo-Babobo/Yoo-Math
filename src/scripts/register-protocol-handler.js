module.exports = () => { if (process.platform === "win32") registerProtocolHandler("yoo-math", "Yoo-Math Launch URL", process.execPath); }
...
function registerProtocolHandler (protocol, name, command) {
  ...
  function callback (err) { if (err) console.error(err.message || err); }
}